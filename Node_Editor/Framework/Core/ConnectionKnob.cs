...
namespace NodeEditorFramework
{
    ...
    public class ConnectionKnob : ConnectionPort
    {
        ...
        public Rect GetCanvasSpaceRemoveKnob(Vector2 startPos, Vector2 endPos)
        {
            Vector2 knobSize = new Vector2(NodeEditorGUI.knobSize * knobAspect, NodeEditorGUI.knobSize);

#if NODE_EDITOR_LINE_CONNECTION
			Vector2 knobCenter = GetConnectionCenterPoint(startPos, endPos, ConnectionDrawMethod.StraightLine);
#else
            Vector2 knobCenter = GetConnectionCenterPoint(startPos, endPos, ConnectionDrawMethod.Bezier);
#endif
            return new Rect(knobCenter.x - knobSize.x / 2, knobCenter.y - knobSize.y / 2, knobSize.x, knobSize.y);
        }

        Vector2 GetConnectionCenterPoint(Vector2 startPos, Vector2 endPos, ConnectionDrawMethod drawMethod)
        {
            //        if (drawMethod == ConnectionDrawMethod.StraightLine)
            //        {
            //            return (startPos + endPos) / 2;
            //        }
            //        else if (drawMethod == ConnectionDrawMethod.Bezier)
            //        {
        ...
        public Vector2 GetKnobCenter(Vector2 knobSize)
        {
